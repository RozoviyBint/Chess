...
namespace ChessEngine.ViewModels
{
    public class CurrentGame : BaseNotificationClass
    {
        ...
        {
            CreateBoard();
            GameState = "Статус игры: В процессе";
            _currentMoves = null;
       ...
       private void ExecuteMove(string Move, string Type)
        ...
        private void EvaluateGameState()
        {
            ...
            else
            {
                if (IsKingChecked)
                {
                    GameState = (Color == PieceColorEnum.Black ? "Статус игры: Белые победили!" : "Статус игры: Черные победили!");
                }
                else
                {
                    GameState = "Статус игры: ПАТ!";
                }
            }
        }
        ...
        {
            if (!String.Equals(GameState, "Статус игры: В процессе")) //guard clause
            {
                return;
            }
